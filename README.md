...
- 对外：单端口 `24040`（Dashboard/Gateway）
- 对内：组合模式（Control Plane / Relayer / Indexer 作为模块挂载）
- 兼容：也支持独立多服务部署（proxy 模式）
...
```bash
curl -X POST http://127.0.0.1:24040/api/intents \
...
- `DASHBOARD_COMPOSITION_MODE=embedded`：默认，单端口内聚模式
- `DASHBOARD_COMPOSITION_MODE=proxy`：网关代理模式（转发到独立服务）

### 端口

- `DASHBOARD_PORT` 或 `POLICYPAY_PORT`：默认 `24040`
- 独立服务模式下可使用：
